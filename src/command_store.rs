...
impl CommandStore {
        ...
        let command_row: CommandTable = match result.next() {
            ...
            Err(
                rusqlite::Error::SqliteFailure(
                    Error {code: ErrorCode::ConstraintViolation, extended_code: 2067 }, _
                )
            ) => {
                // Already exists
                return Ok(false) 
    ...
    pub fn find_cmds_by_name(&self, name: &str) -> anyhow::Result<Vec<Command>> {
        ...
        let mut command_stmt = self.c
            .prepare("SELECT * FROM command WHERE name = ?1")?;
    ...
    pub fn delete_by_id(&self, id: i64) -> anyhow::Result<bool> {
        let mut delete_cmd_stmt = self.c
            .prepare("DELETE FROM command WHERE id = ?1")?;
